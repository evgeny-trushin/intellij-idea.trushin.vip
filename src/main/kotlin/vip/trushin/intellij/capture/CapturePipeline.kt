...
class CapturePipeline(
    ...
    fun captureAction(
        ...
        // Wait briefly for UI to be stable before the "before" screenshot
        Thread.sleep(500)
        ...
        // R6.3: After action - wait for UI to settle, then capture screenshot + UI tree
        // Give the IDE time to process the action and update the UI
        Thread.sleep(1500)
        ...
        // Build and log the event — merge caller-supplied metadata with error info
        val eventMetadata = metadata.toMutableMap()
        if (errorMessage != null) eventMetadata["error"] = errorMessage
